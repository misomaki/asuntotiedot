-- ============================================================
-- Migration 016: Recalibrated age factors
--
-- Validated 2026-03-21 against 87 Etuovi asking prices.
-- New-build factors boosted to compensate for StatFin base price
-- dilution (new + resale transactions pooled in area averages).
-- Half-correction applied to avoid overfitting.
-- Pre-war brackets kept unchanged (validation sample biased).
--
-- Changes from 015:
--   ≤0v: 1.35 → 1.45    ≤50v: 0.82 → 0.84
--   ≤5v: 1.25 → 1.33    ≤60v: 0.78 → 0.80
--   ≤10v: 1.15 → 1.22   ≤70v: unchanged 0.80
--   ≤20v: 1.05 → 1.10   ≤80v: unchanged 0.85
--   ≤30v: 0.95 → 0.97   ≤100v: unchanged 0.90
--   ≤40v: unchanged 0.90 >100v: unchanged 0.92
-- ============================================================

-- Update the main compute_building_price function (used by 05-compute-building-prices.ts)
-- This is a full CREATE OR REPLACE — copy of 015 with updated age factors only.
...
BEGIN
  ...
  -- New-build factors boosted for StatFin base price dilution.
  -- Half-correction to avoid overfitting. Pre-war unchanged.
  -- ========================================================
