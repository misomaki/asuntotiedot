-- ============================================================
-- Migration 030: Sync SQL property type inference with TypeScript
--
-- Fixes two critical sync issues between compute_building_price()
-- and app/lib/priceEstimation.ts:
--
--   1. Property type inference — SQL was missing:
--      - 'apartments' + 2 floors + ≤7 apts → rivitalo
--      - 'residential' + 1 floor + apt_count ≥ 2 → rivitalo
--      - Footprint ≥ 300m² → rivitalo heuristic
--
--   2. OKT_FALLBACK multipliers out of sync:
--      SQL had 1.10/0.90, TypeScript has 1.00/0.75
--
-- After deploying:
...
BEGIN
  -- ========================================================
  -- Property type inference — synced with inferPropertyType()
  -- in app/lib/priceEstimation.ts (2026-04-05)
  -- ========================================================

  -- 1. Ryhti main_purpose (authoritative, highest priority)
  ...
  IF v_property_type IS NULL THEN
    -- 2. Explicit building types
    IF p_building_type = 'apartments' THEN
      -- Low-rise 'apartments' with few units are rivitalo (row houses)
    ...
    ELSIF p_floor_count = 2 THEN
      v_property_type := 'rivitalo';
    ELSIF p_floor_count = 1 AND p_apartment_count IS NOT NULL AND p_apartment_count >= 2 THEN
      -- 1-floor with 2+ apartments → paritalo/rivitalo
      v_property_type := 'rivitalo';
    -- 4. Footprint heuristic — large buildings are likely rivitalo
  ...
  -- OKT_FALLBACK synced with priceEstimation.ts:
  --   fromRivitalo = 1.00, fromKerrostalo = 0.75
  -- ========================================================
  IF v_property_type = 'omakotitalo' THEN
    ...
    IF v_base_price IS NULL THEN
      ...
      LIMIT 1;
    END IF;
  ELSIF v_property_type = 'rivitalo' THEN
    SELECT COALESCE(price_per_sqm_median, price_per_sqm_avg)
    INTO v_base_price
    FROM price_estimates
    WHERE area_id = p_area_id
  ...
  -- ========================================================
  -- Leased plot (vuokratontti) discount
  -- Applied to KT and RT only, not OKT (pending validation)
  -- ========================================================
  v_tontti_factor := 1.0;
  IF p_is_leased_plot = TRUE AND v_property_type != 'omakotitalo' THEN
    v_tontti_factor := 0.92;
  END IF;

  RETURN ROUND(v_base_price * v_age_factor * v_energy_factor * v_water_factor * v_floor_factor * v_size_factor * v_neighborhood_factor * v_tontti_factor);
END;
$$ LANGUAGE plpgsql STABLE;

-- ============================================================
-- Update compute_all_building_prices to pass new params
-- ============================================================
...
BEGIN
  ...
  SET
    estimated_price_per_sqm = compute_building_price(
      ...
      apartment_count, footprint_area_sqm, total_area_sqm, is_leased_plot
    ),
