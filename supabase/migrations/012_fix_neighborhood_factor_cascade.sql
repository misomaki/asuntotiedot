-- ============================================================
-- Remove municipality median from neighborhood factor cascade
-- ============================================================
-- Problem: Migration 011 used municipality median (from high/medium confidence
-- areas) as fallback when an area has no local neighborhood factor data.
-- For Tampere KT, only 3 areas had high/medium confidence factors:
-- Hervanta (1.50), Kaleva (1.12), Vuores (0.98) → median 1.12.
-- This inflated prices by ~12% for ALL Tampere areas without local data
-- (e.g. Härmälä 1960s buildings: 3128 instead of 2773 €/m²).
--
-- Fix: Remove municipality median from neighborhood factor cascade entirely.
-- Areas without n≥3 local data now get 1.0 (neutral) instead of biased median.
-- Municipality median IS still used for base price fallback (separate concern).
--
-- After deploying:
--   1. Reset: npx tsx scripts/data-import/reset-estimation-year.ts
...
BEGIN
  ...
  -- Fixed lookup cascade (2026-03-15):
  --   1. area + exact type, sample_count ≥ 3 (high/medium confidence)
  --   2. area + 'all', sample_count ≥ 3
  --   3. Final: 1.0 (neutral)
  --
  -- Changes from migration 011:
  --   - REMOVED municipality median fallback. With sparse data per city
  --     (e.g. Tampere KT had only 3 high/med areas → median 1.12, biased),
  --     it inflated prices for areas without local data (e.g. Härmälä).
  --   - 1.0 is the safest default when we lack local market data.
  --   - Municipality median IS still used for BASE PRICE fallback (separate).
  -- ========================================================
  v_neighborhood_factor := NULL;

  -- Step 1: exact area + property type, high/medium confidence (≥3 samples)
  ...
  -- Step 2: universal factor for this area, high/medium confidence
  ...
  -- Step 3: Final fallback = 1.0 (neutral)
  IF v_neighborhood_factor IS NULL THEN
  ...
  -- Old buildings (age_factor < 0.85) don't capture neighborhood
  -- or waterfront premiums as strongly as new buildings.
  -- Gradual: age_factor 0.85 → no dampening, 0.70 → 50% dampening
