-- ============================================================
-- Migration 017: Recalibrated water factors + compute missing distances
--
-- Original water factors (max 1.15) were too conservative.
-- Finnish waterfront properties command 25-40% premiums.
-- New 7-tier scale from 1.03 (≤500m) to 1.35 (≤10m).
--
-- Also ensures ALL buildings get a water distance computed.
-- Buildings without centroid or area_id get 99999 (no water effect).
--
-- After deploying:
--   1. Run: npx tsx scripts/data-import/recompute-water-distances.ts
--   2. Reset prices and recompute
-- ============================================================

-- Update compute_building_price with new water factor tiers
-- Full CREATE OR REPLACE — copy of 016 with updated water factors only.
...
BEGIN
  ...
  -- 7-tier scale: Finnish waterfront premiums are 25-40%.
  -- Only lakes >1ha and sea count (filtered in migration 013).
  -- ========================================================
