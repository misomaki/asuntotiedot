-- ============================================================
-- Fix: Municipality-level price fallback for areas without StatFin data
-- ============================================================
-- ~54% of buildings have no price estimate because their postal code
-- area lacks StatFin data entirely. This adds a municipality-level
-- fallback: if no price data exists for the specific area, use the
-- average price from other postal codes in the same municipality.
--
-- After deploying this, re-run price computation:
--   UPDATE buildings SET estimation_year = NULL
--     WHERE estimated_price_per_sqm IS NULL AND area_id IS NOT NULL;
...
BEGIN
  ...
  IF v_base_price IS NULL THEN
    ...
    IF v_municipality IS NOT NULL THEN
      ...
      -- Omakotitalo municipality fallback: rivitalo × 0.85
      IF v_base_price IS NULL AND v_property_type = 'omakotitalo' THEN
        SELECT AVG(COALESCE(pe.price_per_sqm_median, pe.price_per_sqm_avg)) * 0.85
        INTO v_base_price
        FROM price_estimates pe
        JOIN areas a ON a.id = pe.area_id
        WHERE a.municipality = v_municipality
      ...
      -- Omakotitalo municipality fallback: kerrostalo × 0.75
      IF v_base_price IS NULL AND v_property_type = 'omakotitalo' THEN
        SELECT AVG(COALESCE(pe.price_per_sqm_median, pe.price_per_sqm_avg)) * 0.75
        INTO v_base_price
        FROM price_estimates pe
        JOIN areas a ON a.id = pe.area_id
        WHERE a.municipality = v_municipality
  ...
  ELSE
    ...
    v_age_factor := CASE
      ...
      WHEN v_age <= 50 THEN 0.78
      WHEN v_age <= 60 THEN 0.72
      WHEN v_age <= 70 THEN 0.75
      WHEN v_age <= 80 THEN 0.80
